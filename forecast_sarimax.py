...
class Forecast_SARIMAX:
    ...
    def fit(self):
        """
        Fit SARIMAX model using MLE to estimate parameters.

        Attributes:
        - self.dParams: Dictionary of optimal parameters 
        - self.model: Fitted SARIMAX model.
        """

        if self.sFreq == 'D':  # Daily frequency
            order=(7,1,7)
            seasonal_order = None
        elif 'W' in self.sFreq:  # Weekly frequency
            order=(5,1,5)
            # seasonal_order = (1, 0, 1, 52)  # Captures yearly seasonality in weeks
        # else:
        #     seasonal_order = (0, 0, 0, 0)  # No seasonal effect if undefined
        ...
        self.model = SARIMAX(
            ...
            exog=self.exog_vars,  # Include external regressors if available
    ...
    def plot_prediction(self):
        ...
        if self.sFreq== 'D':
            ...
            plt.plot(self.srYhatIS[self.iOoS*20:] , label='In-Sample Forecast', color='blue', linestyle='--')
            plt.plot(self.srYhatOoS, label='Out-of-Sample Forecast', marker='o', color='red', linestyle=':')
            # plt.xlim(self.srYhatIS.index[-self.iOoS*40], self.srYhatIS.index[-1])
